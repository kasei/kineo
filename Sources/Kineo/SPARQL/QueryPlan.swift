...
public protocol QueryPlan: PlanSerializable {
    ...
    func evaluate() throws -> AnyIterator<SPARQLResult<Term>>
...
public struct HashJoinPlan: BinaryQueryPlan {
    ...
    public func evaluate() throws -> AnyIterator<SPARQLResult<Term>> {
        ...
        let r = try rhs.evaluate()
        var table = [SPARQLResult<Term>:[SPARQLResult<Term>]]()
        var unboundTable = [SPARQLResult<Term>]()
...
public struct UnionPlan: BinaryQueryPlan {
    ...
    public func evaluate() throws -> AnyIterator<SPARQLResult<Term>> {
        ...
        let r = try rhs.evaluate()
        var lok = true
        let i = AnyIterator { () -> SPARQLResult<Term>? in
...
public struct ExtendPlan: UnaryQueryPlan {
    ...
    public func evaluate() throws -> AnyIterator<SPARQLResult<Term>> {
        ...
        let s = i.lazy.map { (r) -> SPARQLResult<Term> in
//            evaluator.nextResult()
            do {
                let term = try evaluator.evaluate(expression: expression, result: r)
...
public struct ReducedPlan: UnaryQueryPlan {
    ...
    public func evaluate() throws -> AnyIterator<SPARQLResult<Term>> {
        ...
        let s = try child.evaluate().lazy.compactMap { (r) -> SPARQLResult<Term>? in
            if let l = last, l == r {
                return nil
            }
            last = r
...
public struct WindowPlan: UnaryQueryPlan {
    ...
    private func partition<I: IteratorProtocol>(_ results: I, by partition: [Expression]) -> AnyIterator<AnyIterator<SPARQLResult<Term>>> where I.Element == SPARQLResult<Term> {
        let ee = evaluator
        let seq = AnySequence { return results }
        let withGroups = seq.lazy.map { (r) -> (SPARQLResult<Term>, [Term?]) in
        ...
        let grouped = AnyIterator { () -> AnyIterator<SPARQLResult<Term>>? in
            while true {
    ...
    private func resultsAreEqual(_ a : SPARQLResult<Term>, _ b : SPARQLResult<Term>, usingComparators comparators: [Algebra.SortComparator]) -> Bool {
    ...
    public func evaluate() throws -> AnyIterator<SPARQLResult<Term>> {
        ...
        case .rank, .denseRank:
            ...
            let groups = AnySequence(partitionGroups).lazy.map { (g) -> [SPARQLResult<Term>] in
                var rank = 0
                var increment = 1
                var last: SPARQLResult<Term>? = nil
                let groupResults = g.map { (r) -> SPARQLResult<Term> in
                    if let last = last {
        ...
        case .rowNumber:
            ...
            let groups = AnySequence(partitionGroups).lazy.map { (g) -> [SPARQLResult<Term>] in
                let groupResults = g.lazy.enumerated().map { (i, r) -> SPARQLResult<Term> in
        ...
        case .ntile(let n):
            ...
            let groups = AnySequence(partitionGroups).lazy.map { (g) -> [SPARQLResult<Term>] in
                ...
                var last: SPARQLResult<Term>? = nil
                var groupResults = [SPARQLResult<Term>]()
                var iRow = -1
                for r in sorted {
                    if let last = last {
                        if !self.resultsAreEqual(last, r, usingComparators: order) {
        ...
        case .aggregation(let agg):
            ...
            let groups = try seq.lazy.map { (g) -> [SPARQLResult<Term>] in
        ...
        /**
         ...
         For each group:
            * Buffer each SPARQLResult<Term> in the group
            * create a sequence that will emit a Sequence<SPARQLResult<Term>> with the window value
    ...
    struct SlidingWindowImplementation {
        var add: (SPARQLResult<Term>) -> ()
        var remove: (SPARQLResult<Term>) -> ()
        var value: () -> Term?

        func evaluate<I: IteratorProtocol>(_ group: I, frame: WindowFrame, variableName: String, evaluator: ExpressionEvaluator) throws -> AnyIterator<SPARQLResult<Term>> where I.Element == SPARQLResult<Term> {
        ...
        func evaluateComplete<I: IteratorProtocol>(_ group: I, frame: WindowFrame, variableName: String, evaluator: ExpressionEvaluator) throws -> AnyIterator<SPARQLResult<Term>> where I.Element == SPARQLResult<Term> {
            var group = group
        ...
        func evaluateFromFollowing<I: IteratorProtocol>(_ group: I, frame: WindowFrame, variableName: String, evaluator: ExpressionEvaluator, bound toPrecedingExpression: Expression) throws -> AnyIterator<SPARQLResult<Term>> where I.Element == SPARQLResult<Term> {
            let fflt = try evaluator.evaluate(expression: toPrecedingExpression, result: SPARQLResult<Term>(bindings: [:]))
            ...
            case .following(let expr):
                let t = try evaluator.evaluate(expression: expr, result: SPARQLResult<Term>(bindings: [:]))
            ...
            if let toFollowingLimit = toFollowingLimit {
                ...
                let seq = buffer.map { (r) ->SPARQLResult<Term> in
                    var result = r
                    if let v = self.value() {
                        result = result.extended(variable: variableName, value: v) ?? result
                    }
        ...
        func evaluateToPreceding<I: IteratorProtocol>(_ group: I, frame: WindowFrame, variableName: String, evaluator: ExpressionEvaluator, bound toPrecedingExpression: Expression) throws -> AnyIterator<SPARQLResult<Term>> where I.Element == SPARQLResult<Term> {
            let tplt = try evaluator.evaluate(expression: toPrecedingExpression, result: SPARQLResult<Term>(bindings: [:]))
        ...
        func evaluateFromPrecedingToUnbounded<I: IteratorProtocol>(_ group: I, frame: WindowFrame, variableName: String, evaluator: ExpressionEvaluator, bound fromPrecedingExpression: Expression) throws -> AnyIterator<SPARQLResult<Term>> where I.Element == SPARQLResult<Term> {
            let fplt = try evaluator.evaluate(expression: fromPrecedingExpression, result: SPARQLResult<Term>(bindings: [:]))
        ...
        func evaluateToFollowing<I: IteratorProtocol>(_ group: I, frame: WindowFrame, variableName: String, evaluator: ExpressionEvaluator, bound toPrecedingExpression: Expression) throws -> AnyIterator<SPARQLResult<Term>> where I.Element == SPARQLResult<Term> {
            let tflt = try evaluator.evaluate(expression: toPrecedingExpression, result: SPARQLResult<Term>(bindings: [:]))
        ...
        func evaluateToCurrent<I: IteratorProtocol>(_ group: I, frame: WindowFrame, variableName: String, evaluator: ExpressionEvaluator) throws -> AnyIterator<SPARQLResult<Term>> where I.Element == SPARQLResult<Term> {
        ...
        func evaluateFromCurrent<I: IteratorProtocol>(_ group: I, frame: WindowFrame, variableName: String, evaluator: ExpressionEvaluator) throws -> AnyIterator<SPARQLResult<Term>> where I.Element == SPARQLResult<Term> {
...
public struct ExistsPlan: UnaryQueryPlan {
    ...
    public func evaluate() throws -> AnyIterator<SPARQLResult<Term>> {
        ...
        let variable = self.variable
        let s = i.lazy.compactMap { (r) -> SPARQLResult<Term>? in
...
public protocol PathPlan: PlanSerializable {
    ...
    func evaluate(from: Node, to: Node, in: Term) throws -> AnyIterator<SPARQLResult<Term>>
}
...
public struct NPSPathPlan: PathPlan {
    ...
    public func evaluate(from subject: Node, to object: Node, in graph: Term) throws -> AnyIterator<SPARQLResult<Term>> {
        switch (subject, object) {
        case let (.bound(_), .variable(oname, binding: bind)):
            let objects = try evaluate(from: subject, in: graph)
            if bind {
                let i = objects.lazy.map {
                    return SPARQLResult<Term>(bindings: [oname: $0])
                }
                return AnyIterator(i.makeIterator())
            } else {
                let i = objects.lazy.map { (_) -> SPARQLResult<Term> in
                    return SPARQLResult<Term>(bindings: [:])
                }
                return AnyIterator(i.makeIterator())
            }
        case (.bound(_), .bound(let o)):
            let objects = try evaluate(from: subject, in: graph)
            let i = objects.lazy.compactMap { (term) -> SPARQLResult<Term>? in
        ...
        case (.variable(let s, _), .bound(_)):
            ...
            return AnyIterator {
                repeat {
                    ...
                    return SPARQLResult<Term>(bindings: [s: q.subject])
                } while true
            }
        case let (.variable(s, _), .variable(o, _)):
            ...
            return AnyIterator {
                repeat {
                    ...
                    let p = q.predicate
                    guard !set.contains(p) else { continue }
                    return SPARQLResult<Term>(bindings: [s: q.subject, o: q.object])
                } while true
            }
        }
    }
...
//            print("ALP resulted in: \(v)")
            ...
            let i = v.lazy.map { (term) -> SPARQLResult<Term> in
                if case .variable(let name, true) = object {
                    return SPARQLResult<Term>(bindings: [name: term])
                } else {
                    return SPARQLResult<Term>(bindings: [:])
                }
            }
...
//            print("ALP resulted in: \(v)")
            ...
            case let .variable(name, binding: true):
                let i = v.lazy.map { SPARQLResult<Term>(bindings: [name: $0]) }
                return AnyIterator(i.makeIterator())
            case .variable(_, binding: false):
                let i = v.lazy.map { (_) in SPARQLResult<Term>(bindings: [:]) }.prefix(1)
                return AnyIterator(i.makeIterator())
            case .bound(let o):
                let i = v.lazy.compactMap { (term) -> SPARQLResult<Term>? in
                    guard term == o else { return nil }
                    return SPARQLResult<Term>(bindings: [:])
                }
        ...
        case .variable(let s, binding: _):
            ...
            case .variable(_):
                ...
                for gn in store.graphTerms(in: graph) {
                    let results = try evaluate(from: .bound(gn), to: object, in: graph).lazy.compactMap { (r) -> SPARQLResult<Term>? in
...
public struct ZeroOrOnePathPlan : PathPlan {
    ...
    public func evaluate(from subject: Node, to object: Node, in graph: Term) throws -> AnyIterator<SPARQLResult<Term>> {
        ...
        case (.variable(let s, _), .variable(_)):
            ...
            let results = try gn.lazy.map { (term) -> AnyIterator<SPARQLResult<Term>> in
                let i = try child.evaluate(from: .bound(term), to: object, in: graph)
                let j = i.lazy.map { (r) -> SPARQLResult<Term> in
        ...
        case (.bound(_), .bound(_)):
            if subject == object {
                let r = [SPARQLResult<Term>(bindings: [:])]
                return AnyIterator(r.makeIterator())
            } else {
                return i
            }
        case let (.bound(term), .variable(name, _)), let (.variable(name, _), .bound(term)):
            let r = [SPARQLResult<Term>(bindings: [name: term])]
            var seen = Set<Term>()
            let j = i.lazy.compactMap { (r) -> SPARQLResult<Term>? in
...
protocol Aggregate {
    func handle(_ row: SPARQLResult<Term>)
    func result() -> Term?
}
...
public struct AggregationPlan: UnaryQueryPlan {
    ...
    public func evaluate() throws -> AnyIterator<SPARQLResult<Term>> {
        ...
        let rows = aggData.map { (group, aggs) -> SPARQLResult<Term> in
            ...
            return SPARQLResult<Term>(bindings: result)
        }
        return AnyIterator(rows.makeIterator())
    }
}
