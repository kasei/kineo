...
public struct QueryPlanEvaluator<Q: QuadStoreProtocol>: QueryEvaluatorProtocol {
    public typealias ResultSequence = AnySequence<SPARQLResult<Term>>
    ...
    public func evaluate(query: Query) throws -> QueryResult<AnySequence<SPARQLResult<Term>>, [Triple]> {
        return try evaluate(query: query, activeGraph: nil)
    }
    
    public func evaluate(query: Query, activeGraph graph: Term? = nil) throws -> QueryResult<AnySequence<SPARQLResult<Term>>, [Triple]> {
...
//        print(plan.serialize())
        ...
        let seq = AnySequence { () -> AnyIterator<SPARQLResult<Term>> in
            do {
