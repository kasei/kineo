...
//    typealias TestedQuadStore = MemoryQuadStore
    ...
    func evaluate<QE: QueryEvaluatorProtocol>(query: Query, in store: TestedQuadStore, dataset: Dataset, defaultGraph: Term, evaluator e: QE) throws -> QueryResult<[SPARQLResultSolution<Term>], [Triple]> {
        do {
...
//            print("Successful query plan evaluation")
    ...
    func booleanResults<Q: QuadStoreProtocol>(from quadstore: Q, defaultGraph: Term) throws -> QueryResult<[SPARQLResultSolution<Term>], [Triple]> {
    ...
    func bindingResults<Q: QuadStoreProtocol>(from quadstore: Q, defaultGraph: Term) throws -> QueryResult<[SPARQLResultSolution<Term>], [Triple]> {
        ...
        for solution in solutions {
            ...
            let result = SPARQLResultSolution<Term>(bindings: d)
            results.append(result)
        }
        return QueryResult<[SPARQLResultSolution<Term>], [Triple]>.bindings(vars, results)
    }
    
    func expectedResults(for query: Query, from url: URL) throws -> QueryResult<[SPARQLResultSolution<Term>], [Triple]> {
        if url.absoluteString.hasSuffix("srx") {
            ...
            case .construct(_), .describe(_):
                return QueryResult<[SPARQLResultSolution<Term>], [Triple]>.triples(triples)
            default:
    ...
    func manifestSyntaxItems<Q: QuadStoreProtocol>(quadstore: Q, manifest: Term, type: Term? = nil) throws -> AnyIterator<SPARQLResultSolution<Term>> {
        if verbose {
