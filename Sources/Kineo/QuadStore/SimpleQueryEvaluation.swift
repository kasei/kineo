...
public protocol SimpleQueryEvaluatorProtocol: QueryEvaluatorProtocol {
    ...
    func evaluate(query: Query) throws -> QueryResult<[SPARQLResult<Term>], [Triple]>
    func evaluate(query: Query, activeGraph: Term?) throws -> QueryResult<[SPARQLResult<Term>], [Triple]>
    func evaluate(algebra: Algebra, activeGraph: Term?) throws -> AnyIterator<SPARQLResult<Term>>
    func evaluateTable(columns names: [Node], rows: [[Term?]]) throws -> AnyIterator<SPARQLResult<Term>>
    func evaluateSlice(_ i: AnyIterator<SPARQLResult<Term>>, offset: Int?, limit: Int?) throws -> AnyIterator<SPARQLResult<Term>>
    func evaluateExtend(_ i: AnyIterator<SPARQLResult<Term>>, expression expr: Expression, name: String) throws -> AnyIterator<SPARQLResult<Term>>
    func evaluateFilter(_ i: AnyIterator<SPARQLResult<Term>>, expression expr: Expression, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>>
    func evaluateMinus(_ l: AnyIterator<SPARQLResult<Term>>, _ r: [SPARQLResult<Term>]) throws -> AnyIterator<SPARQLResult<Term>>
    func evaluate(algebra: Algebra, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>>
    ...
    func evaluateUnion(_ patterns: [Algebra], activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>>
    func evaluateJoin(lhs lhsAlgebra: Algebra, rhs rhsAlgebra: Algebra, left: Bool, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>>
    func evaluate(diff lhs: Algebra, _ rhs: Algebra, expression expr: Expression, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>>
    func evaluateLeftJoin(lhs: Algebra, rhs: Algebra, expression expr: Expression, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>>
    func evaluate(algebra: Algebra, endpoint: URL, silent: Bool, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>>
    func evaluateCount<S: Sequence>(results: S, expression keyExpr: Expression, distinct: Bool) -> Term? where S.Iterator.Element == SPARQLResult<Term>
    func evaluateCountAll<S: Sequence>(results: S) -> Term? where S.Iterator.Element == SPARQLResult<Term>
    func evaluateAvg<S: Sequence>(results: S, expression keyExpr: Expression, distinct: Bool) -> Term? where S.Iterator.Element == SPARQLResult<Term>
    func evaluateSum<S: Sequence>(results: S, expression keyExpr: Expression, distinct: Bool) -> Term? where S.Iterator.Element == SPARQLResult<Term>
    func evaluateGroupConcat<S: Sequence>(results: S, expression keyExpr: Expression, separator: String, distinct: Bool) -> Term? where S.Iterator.Element == SPARQLResult<Term>
    func evaluateWindow(algebra child: Algebra, function: Algebra.WindowFunctionMapping, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>>
    func evaluatePath(subject: Node, object: Node, graph: Term, path: PropertyPath) throws -> AnyIterator<SPARQLResult<Term>>
    func evaluateAggregation(algebra child: Algebra, groups: [Expression], aggregations aggs: Set<Algebra.AggregationMapping>, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>>
    func evaluateSort<S: Sequence>(_ results: S, comparators: [Algebra.SortComparator]) -> [SPARQLResult<Term>] where S.Element == SPARQLResult<Term>
    func resultsAreEqual(_ a : SPARQLResult<Term>, _ b : SPARQLResult<Term>, usingComparators: [Algebra.SortComparator]) -> Bool
    
    func evaluate(bgp: [TriplePattern], activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>>
    
    func evaluate(quad: QuadPattern) throws -> AnyIterator<SPARQLResult<Term>>
    func evaluate(algebra: Algebra, inGraph: Node) throws -> AnyIterator<SPARQLResult<Term>>
...
extension SimpleQueryEvaluatorProtocol {
    public func evaluate(query original: Query) throws -> QueryResult<[SPARQLResult<Term>], [Triple]> {
...
//        }
    ...
    public func evaluate(query: Query, activeGraph: Term?) throws -> QueryResult<[SPARQLResult<Term>], [Triple]> {
        ...
        case .select(_):
            ...
            let r : QueryResult<[SPARQLResult<Term>], [Triple]> = QueryResult.bindings(variables, results)
            return r
    ...
    public func evaluate(algebra: Algebra, activeGraph: Term? = nil) throws -> AnyIterator<SPARQLResult<Term>> {
        var iterators = [AnyIterator<SPARQLResult<Term>>]()
        let graphs : [Term]
    ...
    public func evaluateTable(columns names: [Node], rows: [[Term?]]) throws -> AnyIterator<SPARQLResult<Term>> {
    ...
    public func evaluateSlice(_ i: AnyIterator<SPARQLResult<Term>>, offset: Int?, limit: Int?) throws -> AnyIterator<SPARQLResult<Term>> {
    ...
    public func evaluateExtend(_ i: AnyIterator<SPARQLResult<Term>>, expression expr: Expression, name: String) throws -> AnyIterator<SPARQLResult<Term>> {
        if expr.isNumeric {
            return AnyIterator {
    ...
    public func evaluateFilter(_ i: AnyIterator<SPARQLResult<Term>>, expression expr: Expression, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>> {
        return AnyIterator {
            repeat {
    ...
    public func evaluateMinus(_ l: AnyIterator<SPARQLResult<Term>>, _ r: [SPARQLResult<Term>]) throws -> AnyIterator<SPARQLResult<Term>> {
        return AnyIterator {
            while true {
    ...
    public func evaluate(algebra: Algebra, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>> {
        switch algebra {
        ...
        case let .reduced(child):
            let i = try self.evaluate(algebra: child, activeGraph: activeGraph)
            var last: SPARQLResult<Term>? = nil
            return AnyIterator {
                repeat {
                    guard let result = i.next() else { return nil }
        ...
        case let .distinct(child):
            let i = try self.evaluate(algebra: child, activeGraph: activeGraph)
            var seen = Set<SPARQLResult<Term>>()
            return AnyIterator {
                repeat {
                    guard let result = i.next() else { return nil }
    ...
    public func evaluateUnion(_ _patterns: [Algebra], activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>> {
        var patterns = _patterns
        var currentIterator: AnyIterator<SPARQLResult<Term>>? = nil
        return AnyIterator { () -> SPARQLResult<Term>? in
            do {
                repeat {
...
//    public func evaluateUnion(_ patterns: [Algebra], activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>> {
...
//    }
    ...
    public func evaluateJoin(lhs lhsAlgebra: Algebra, rhs rhsAlgebra: Algebra, left: Bool, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>> {
        var seen = [Set<String>]()
        ...
        var patternResults = [[SPARQLResult<Term>]]()
        for pattern in [lhsAlgebra, rhsAlgebra] {
    ...
    public func evaluate(diff lhs: Algebra, _ rhs: Algebra, expression expr: Expression, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>> {
    ...
    public func evaluateLeftJoin(lhs: Algebra, rhs: Algebra, expression expr: Expression, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>> {
    ...
    public func evaluate(algebra: Algebra, endpoint: URL, silent: Bool, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>> {
    ...
    public func evaluateCount<S: Sequence>(results: S, expression keyExpr: Expression, distinct: Bool) -> Term? where S.Iterator.Element == SPARQLResult<Term> {
        if distinct {
    ...
    public func evaluateAggregation<S: Sequence>(_ agg: Aggregation, group results: S) -> Term? where S.Iterator.Element == SPARQLResult<Term> {
        switch agg {
        case .countAll:
    ...
    public func evaluateCountAll<S: Sequence>(results: S) -> Term? where S.Iterator.Element == SPARQLResult<Term> {
        var count = 0
        for _ in results {
            count += 1
        }
    ...
    public func evaluateAvg<S: Sequence>(results: S, expression keyExpr: Expression, distinct: Bool) -> Term? where S.Iterator.Element == SPARQLResult<Term> {
    ...
    public func evaluateSum<S: Sequence>(results: S, expression keyExpr: Expression, distinct: Bool) -> Term? where S.Iterator.Element == SPARQLResult<Term> {
    ...
    public func evaluateGroupConcat<S: Sequence>(results: S, expression keyExpr: Expression, separator: String, distinct: Bool) -> Term? where S.Iterator.Element == SPARQLResult<Term> {
        var terms = results.map { try? self.ee.evaluate(expression: keyExpr, result: $0) }.compactMap { $0 }
        if distinct {
            terms = Set(terms).sorted()
        }
    ...
    internal func evaluateSinglePipelinedAggregation(algebra child: Algebra, groups: [Expression], aggregation agg: Aggregation, variable name: String, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>> {
        let i = try self.evaluate(algebra: child, activeGraph: activeGraph)
        ...
        if numericGroups.count == 0 && termGroups.count == 0 {
            ...
            case .avg, .count, .countAll, .sum:
                ...
                let result = SPARQLResult<Term>(bindings: [name: Term(integer: 0)])
                return AnyIterator([result].makeIterator())
            default:
                let result = SPARQLResult<Term>(bindings: [:])
                return AnyIterator([result].makeIterator())
            }
        }
        
        var a = numericGroups.makeIterator()
        let numericIterator : AnyIterator<SPARQLResult<Term>> = AnyIterator {
        ...
        let termIterator : AnyIterator<SPARQLResult<Term>> = AnyIterator {
    ...
    public func evaluateWindow(algebra child: Algebra, function windowMap: Algebra.WindowFunctionMapping, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>> {
        let i = try self.evaluate(algebra: child, activeGraph: activeGraph)
        var groupBuckets = [String:[SPARQLResult<Term>]]()
        ...
        let results = try groups.map { (results) -> [SPARQLResult<Term>] in
            var newResults = [SPARQLResult<Term>]()
    ...
    public func evaluatePath(subject: Node, object: Node, graph: Term, path: PropertyPath) throws -> AnyIterator<SPARQLResult<Term>> {
        switch path {
        ...
        case .plus(let pp):
            ...
            case (.bound(_), .variable(let oname, binding: _)):
                ...
                return AnyIterator {
                    ...
                    let r = SPARQLResult<Term>(bindings: [oname: t])
                    return r
                }
            ...
            case (.bound(_), .bound(let oterm)):
                ...
                var results = [SPARQLResult<Term>]()
                if v.contains(oterm) {
                    results.append(SPARQLResult<Term>(bindings: [:]))
                }
                return AnyIterator(results.makeIterator())
        ...
        case .star(let pp):
            ...
            case (.bound(let t), .variable(let oname, binding: _)):
                ...
                return AnyIterator {
                    ...
                    let r = SPARQLResult<Term>(bindings: [oname: o])
                    return r
                }
            case (.variable(_), .bound(_)):
        ...
        case .zeroOrOne(let pp):
            ...
            case (.bound(let x), .variable(let oname, binding: _)):
                ...
                var results = Set<SPARQLResult<Term>>()
                results.insert(SPARQLResult<Term>(bindings: [oname: x]))
            ...
            case (.variable(let sname, binding: _), .bound(let y)):
                ...
                var results = Set<SPARQLResult<Term>>()
                results.insert(SPARQLResult<Term>(bindings: [sname: y]))
                let i = try evaluatePath(subject: subject, object: object, graph: graph, path: pp)
                results.formUnion(i)
                return AnyIterator(results.makeIterator())
            ...
            case (.variable(let sname, binding: _), .variable(let oname, binding: _)):
                ...
                var results = [SPARQLResult<Term>]()
                for t in evaluateGraphTerms(in: graph) {
                    results.append(SPARQLResult<Term>(bindings: [sname: t, oname: t]))
                }
                let i = try evaluatePath(subject: subject, object: object, graph: graph, path: pp)
    ...
    internal func evaluateNPS(subject: Node, object: Node, graph: Term, not iris: [Term]) throws -> AnyIterator<SPARQLResult<Term>> {
        ...
        //           and allowing it to do the filtering based on a SPARQLResult<UInt64> objects before
    ...
    public func evaluateAggregation(algebra child: Algebra, groups: [Expression], aggregations aggs: Set<Algebra.AggregationMapping>, activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>> {
        let i = try self.evaluate(algebra: child, activeGraph: activeGraph)
        var groupBuckets = [String:[SPARQLResult<Term>]]()
        var groupBindings = [String:[String:Term]]()
    ...
    private func comparisonTerms(from term: SPARQLResult<Term>, using comparators: [Algebra.SortComparator]) -> [Term?] {
    ...
    public func resultsAreEqual(_ a : SPARQLResult<Term>, _ b : SPARQLResult<Term>, usingComparators comparators: [Algebra.SortComparator]) -> Bool {
    ...
    public func evaluateSort<S: Sequence>(_ results: S, comparators: [Algebra.SortComparator]) -> [SPARQLResult<Term>] where S.Element == SPARQLResult<Term> {
    ...
    public func evaluate(bgp patterns: [TriplePattern], activeGraph: Term) throws -> AnyIterator<SPARQLResult<Term>> {
    ...
    public func triples<S : Sequence>(describing node: Node, from results: S) throws -> AnyIterator<Triple> where S.Element == SPARQLResult<Term> {
        switch node {
        case .bound(let term):
    ...
    public func triples<S : Sequence>(from results: S, with template: [TriplePattern]) -> [Triple] where S.Element == SPARQLResult<Term> {
...
open class SimpleQueryEvaluator<Q: QuadStoreProtocol>: SimpleQueryEvaluatorProtocol {
    ...
    public func evaluate(quad: QuadPattern) throws -> AnyIterator<SPARQLResult<Term>> {
    ...
    public func evaluate(algebra child: Algebra, inGraph graph: Node) throws -> AnyIterator<SPARQLResult<Term>> {
