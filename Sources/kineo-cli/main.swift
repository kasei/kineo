...
func load<Q: MutableQuadStoreProtocol>(store: Q, configuration config: QuadStoreConfiguration) throws -> Int {
    ...
    if case let .loadFiles(defaultGraphs, namedGraphs) = config.initialize {
        ...
        count += try parse(into: store, files: defaultGraphs, version: startSecond, graph: defaultGraph)
        
        for (graph, file) in namedGraphs {
            print("Loading RDF file into named graph \(graph): \(file)")
            count = try parse(into: store, files: [file], version: startSecond, graph: graph)
        }
    }
    return count
}
...
func parse<Q: MutableQuadStoreProtocol>(into store: Q, files: [String], version: Version, graph defaultGraphTerm: Term? = nil) throws -> Int {
    var count = 0
    for filename in files {
        #if os (OSX)
        guard let path = NSURL(fileURLWithPath: filename).absoluteString else { throw DatabaseError.DataError("Not a valid graph path: \(filename)") }
...
func explain<Q : QuadStoreProtocol>(in store: Q, query: Query, graph: Term? = nil, verbose: Bool) throws {
    ...
    let planner     = QueryPlanner(store: store, dataset: store.dataset(withDefault: defaultGraph))
    ...
    try config.withStore { (qs) throws in
        if case let .loadFiles(defaultFiles, namedFiles) = config.initialize {
            if let mqs = qs as? SQLiteQuadStore {
                count += try parse(into: mqs, files: defaultFiles, version: startSecond, graph: nil)
                try namedFiles.forEach { (graph, file) throws in
                    count += try parse(into: mqs, files: [file], version: startSecond, graph: graph)
                }
